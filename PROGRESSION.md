...
```
prestige_currency_gained = floor( cube_root( (total_earned_this_run + currency) / 100_000 ) )
```
...
Cookie Clicker 方式（cube_root + 累計＋手元）。立方根なので**序盤は厚め、後半は穏やかに**伸びる。手元¥は実質二重カウントになる（抱え込みにも報酬）。閾値とdivisorを同じ 100K にしているので「初到達 = ピッタリ1個」のクリーンな下限。
...
| **メタタブの解放条件** | 累計獲得¥ ≥ 100,000 を**一度でも**満たした瞬間に解放、以後常時表示 |
| **`prestige_count` の意味** | ReactionRule.min_tier に直結（別 Tier 変数は作らない） |
| **コスト式** | 上記 Type A / Type B、`base * growth^lv` 指数式で統一 |
| **`prestige_currency_gained`** | `floor( cube_root( (total_earned_this_run + currency) / 100_000 ) )` Cookie Clicker 方式（cube_root + 累計＋手元）。閾値とdivisorを同じ 100K にして初到達=1個 |
