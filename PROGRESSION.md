...
**ショップ陳列はステージ非依存。買うのは自由。** ただし **「使えるかどうか」は信頼度ステージ依存**。

- 信頼度ステージ < 2 → 使おうとしても拒絶（既存 `LOCKED_OUT` リアクション）
- ステージ 2 = 警戒解除、軽度の DIRECT 系は受け入れ
- ステージ 3 = ROMANCE / DIRECT_TOY が機能
- ステージ 4 = DIRECT_BIND / DIRECT_DRUG が機能

実装は **ReactionRule 側の `min_trust`** で素直に表現できる（既存仕組み）。
「買ったけど使えない」という焦らしが物語的に綺麗で、紳士枠の建前 → 解放 → 落ちる、の流れに乗る。
