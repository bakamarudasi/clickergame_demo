...
```
prestige_currency_gained = floor(sqrt((total_earned_this_run + currency) / 1_000_000))
```
...
| **`prestige_currency_gained`** | `floor(sqrt((total_earned_this_run + currency) / 1_000_000))` Cookie Clicker 方式。手元¥は実質二重カウントされ、抱え込みにも報酬が出る |
