...
### 3タブ構成（横長 1280x720・L字レイアウト）

ビューポートは **1280x720（横持ち）**。上端に StatusBar（64px）、左端に Sidebar（140px）、残りの 1140x656 がタブごとの本体。各タブ内部はさらに HBox 主体で左右分割するパターンが基本（Workは 書類＋強化、Roomは 一覧＋立ち絵＋詳細、Shopは 一覧＋詳細）。

| タブ | 役割 | 主機能 | 内部分割 |
|---|---|---|---|
| **Work** | 通貨生成 | 書類クリック、強化購入 | 左: DocumentButton ／ 右: 強化リスト＋詳細 |
| **Room** | キャラ触れ合い | 立ち絵、ギフト、タッチ、検査、紳士眼鏡、Memory | 左: オペ一覧 ／ 中央: 立ち絵＋スコープ ／ 右: 詳細＆操作 |
| **Shop** | アイテム購入 | カテゴリ別ショップ、衣装、ルール、紳士眼鏡 | 上: カテゴリ ／ 下左: アイテム一覧 ／ 下右: 詳細＆購入 |
...
scenes/
  main.tscn               L字レイアウト枠（StatusBar 上 + Sidebar 左 + TabHolder）
  ui/                     タブ毎 .tscn
translations/
